...
<head>
    ...
    <!-- css -->
    <link rel="stylesheet" href="Custom/catagorys.css">
...
<nav class="navbar navbar-dark bg-dark nav-head slide-right">
  <div class="container-fluid">
    <a class="navbar-brand" href="index.php" style="margin-left: 150px;">
      <img src="image/logo01.png" alt="" width="60" height="60" class="d-inline-block align-text-top" style="margin-right: 10px;">
...
<?php foreach($result as $row) { ?>
    <li class="dropdown btn-group" style="margin-left: 25px;">
        <a href="javascript:void(0);" onclick="toggleDropdown(<?php echo $row['type_id']; ?>)" style="width:95%; margin: auto; padding: 25px; margin-bottom: 10px; text-align: left;" class="btn btn-button" type="button">
        ...
        <div id="dropdownContent_<?php echo $row['type_id']; ?>" class="dropdown-sub_type btn-group dropend" style="display: none;">
            <?php
                ...
                foreach ($result_sub_type as $row_sub_type) {
                    echo '<a class="dropdown-item" href="insert_post.php?act=showbytype&type_id='.$row['type_id'].'&sub_type_id=' . $row_sub_type['sub_type_id'] . '">' . $row_sub_type['sub_type_name'] . '</a>';
                }
            ?>
        </div>
    </li>
